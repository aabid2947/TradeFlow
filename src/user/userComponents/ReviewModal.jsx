...
const Select = ({ children, onValueChange, value }) => (
  <div className="relative">
    <select
      ...
      value={value}
    >
      <option value="" disabled>Select a completed transaction</option>
      {children}
    </select>
  </div>
);
...
export default function ReviewModal({ existingReview, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEditing && !selectedTransaction) {
      setError("Please select the transaction you want to review.");
      return;
    }
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-lg mx-auto relative h-[90vh]"
        >
                ...
                {!isEditing && (
                  ...
                  >
                    <div className="block text-sm font-semibold text-gray-700 mb-3">
                      Which purchase is this review for? 📦
                    </div>
                    <div className="relative">
                      <Select onValueChange={setSelectedTransaction} value={selectedTransaction}>
                        <SelectContent>
                          {unreviewedTransactions.length > 0 ? (
                            ...
                            <option disabled>No unreviewed purchases found.</option>
                          )}
                        </SelectContent>
                      </Select>
                    </div>
