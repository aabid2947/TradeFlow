// src/components/HomePage/ServiceCard.jsx
...
// Helper Animation Components (same as your original code)
...
export const GovernmentIdVisual = () => (
  <div className="relative bg-gradient-to-br from-purple-500 via-[#1987BF] to-blue-600 p-8 lg:p-12 flex items-center justify-center rounded-xl h-[32rem] ">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-end">
              ...
              <div className="text-right">
                <div className="text-xs font-bold text-[#1987BF]">VERIFIED</div>
...
export const CompanyCredentialVisual = () => (
  <div className="relative bg-gradient-to-br from-gray-800 via-gray-700 to-[#1987BF] p-8 lg:p-12 flex items-center justify-center h-[32rem] rounded-xl">
    <div className="absolute inset-0 overflow-hidden">
      <FloatingElement delay={0} className="absolute top-8 right-8 w-20 h-20 bg-white/10 rounded-2xl" />
      <FloatingElement delay={1.5} className="absolute bottom-8 left-8 w-16 h-16 bg-[#1987BF]/30 rounded-full" />
    </div>
    <div className="relative z-10 max-w-md w-full">
      <div className="relative">
        <div className="bg-white/95 backdrop-blur-lg rounded-2xl shadow-2xl p-6 animate-pulse-glow">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[#1987BF] rounded-lg flex items-center justify-center"> <Building2 className="w-6 h-6 text-white" /> </div>
              <div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {[1, 2, 3, 4].map((i) => ( <div key={i} className="bg-gray-50 rounded-lg p-3"> <div className="h-2 bg-gray-200 rounded w-full mb-2"></div> <div className={`h-4 bg-[#1987BF] rounded w-3/4`}></div> </div> ))}
          </div>
          <div className="space-y-3">
        ...
        <div className="absolute -bottom-4 -right-4 bg-[#1987BF] text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1"> <Zap className="w-3 h-3" /> INSTANT </div>
      </div>
    </div>
  </div>
);

// Added new visuals for other cards for variety
export const BankVisual = () => (
    <div className="relative bg-gradient-to-br from-emerald-500 to-green-600 p-8 lg:p-12 flex items-center justify-center h-full">
        {/* Visual content for Bank Verification */}
        <div className="text-white text-3xl font-bold">Bank Verification Visual</div>
    </div>
);

export const EducationVisual = () => (
    <div className="relative bg-gradient-to-br from-red-500 to-amber-600 p-8 lg:p-12 flex items-center justify-center h-full">
        {/* Visual content for Education Verification */}
        <div className="text-white text-3xl font-bold">Education Verification Visual</div>
    </div>
);


// The Reusable Child Component
export default function ServiceCard({ data }) {
  const [isHovered, setIsHovered] = useState(false);
  const { layout, theme, badgeIcon: BadgeIcon, badgeText, titleLines, typewriterTexts, features, ctaText, VisualComponent } = data;
  ...
  return (
    <>
      {/* The style block is now part of the reusable component */}
      <style jsx>{`
        @keyframes float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-15px); } }
        @keyframes pulse-glow { 0%, 100% { box-shadow: 0 0 20px ${theme.primary}33; } 50% { box-shadow: 0 0 40px ${theme.primary}66; } }
        .animate-float { animation: float 6s ease-in-out infinite; }
        .animate-pulse-glow { animation: pulse-glow 3s ease-in-out infinite; }
      `}</style>
      
      <Card
        className={`overflow-hidden border-0 shadow-2xl transition-all duration-500 hover:shadow-3xl ${isHovered ? "scale-[1.02]" : ""}`}
      ...
      >
        <CardContent className="p-0">
          <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[550px]">
            {/* Text Content */}
            <div className={`p-8 lg:p-12 flex flex-col justify-center relative ${contentOrder}`}>
              <div className="relative z-10">
                <div className={`inline-flex items-center gap-2 ${theme.badgeBg} ${theme.badgeText} px-4 py-2 rounded-full text-sm font-semibold mb-6 animate-pulse-glow`}>
                ...
                <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-6 leading-tight">
                  {titleLines.map((line, index) => (
                    <React.Fragment key={index}>
                      <AnimatedText text={line} delay={400 + index * 400} className={index % 2 !== 0 ? `text-[${theme.primary}]` : ''} />
                ...
                <div className="space-y-3 mb-8">
                  {features.map((feature, index) => (
                    <div key={index} className="flex items-center gap-3 opacity-0 animate-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${1400 + index * 200}ms`, animationFillMode: "forwards" }}>
                      <CheckCircle className="w-5 h-5 text-green-500" />
                      <span className="text-gray-700 font-medium">{feature}</span>
                    </div>
                  ))}
                </div>
                <Button className={`group ${theme.gradientFrom} ${theme.gradientTo} text-white px-8 py-4 text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105`}>
