...
export default function AddServiceForm({ onSubmit, onClose, isLoading, error }) {
        ...
        <div className="space-y-4">
            ...
            {service[fieldType].map((field, index) => (
                <div key={index} className="p-4 border border-gray-200 rounded-md space-y-3 relative">
                     <button
                        type="button"
                        onClick={() => removeField(index, fieldType)}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    {error && (
                        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                            ...
                            <span className="block sm:inline">{error.data?.message || 'An unknown error occurred.'}</span>
                        </div>
                    )}
                    ...
                    <div className="p-4 border border-gray-200 rounded-md space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Input label="Service Name" name="name" value={service.name} onChange={handleChange} placeholder="e.g., PAN Verification" required />
                            <Input label="Service Key" name="service_key" value={service.service_key} onChange={handleChange} placeholder="Unique key, e.g., PAN_VERIFY_V1" required />
                            <Input label="Price" name="price" type="number" value={service.price} onChange={handleChange} required />
                            <Input label="Image URL" name="imageUrl" value={service.imageUrl} onChange={handleChange} placeholder="https://example.com/image.png" />
                        </div>
                        <div>
                            <label htmlFor="description" className="block text-sm font-medium text-gray-700">Description</label>
                            <textarea id="description" name="description" value={service.description} onChange={handleChange} rows="3" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="A brief description of the service." required></textarea>
                        </div>
                    </div>
                    ...
                    <div className="p-4 border border-gray-200 rounded-md space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Input label="API Endpoint" name="endpoint" value={service.endpoint} onChange={handleChange} placeholder="e.g., pan-api/fetch-details" required />
